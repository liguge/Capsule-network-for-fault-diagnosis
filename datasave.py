    ...
    def get_data(self):
        ...
        for i in range(len(file_list)):
            ...
            for j in range(0, len(data1)-1023, 400):
                  # print(data1[j:j+1024].reshape(32, 32)[np.newaxis,:,:].shape)#
                  data2 = ss.fit_transform(data1[j:j + 1024]).T
                  x = np.concatenate((x, data2.reshape(32, 32)[np.newaxis, :, :]), axis=0)
            #else:
                #data = np.hstack((data, file[key][0:10]))
        # print(x.shape)
        # print(x[1:2001, :, :].shape)
        return x[1:2001, :, :]
    def get_label(self):
        file_list = self.file_list()
...
Train = TrainDataset()
Test = TestDataset()
train_loader = da.DataLoader(Train, batch_size=32, shuffle=True)#batch_size可以直接设置，用于调整参数。
